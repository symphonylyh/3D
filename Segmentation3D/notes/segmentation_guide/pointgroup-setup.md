...
* Input to the Backbone U-Net is voxelized data (so the dimension of data is greatly reduced), and the output features are voxel features. By indexing the p2v_map, points in the same voxel have the same features. Accordingly, the semantic_pred and pt_offset are calculated based on voxel feature --> points in the same voxel have exactly the same semantic_pred and pt_offset.
* Similarly, ScoreNet works on voxelized data as well, so before passing clusters to ScoreNet, each cluster is voxelized first. Shouldn't this have smaller voxel size?
