...
class PointGroup(nn.Module):
    ...
    def generate_proposal(self, mask_logits, batch_id, threshold, seg_pred, inst_pred_seg_label, min_pts_num=50):
        ...
        for n in range(n_inst):
            ...
            proposal_id_n = ((mask_logits[n] > threshold) & (seg_pred == inst_pred_seg_label[n].item())).nonzero().squeeze(dim=1) # issue: https://github.com/aim-uofa/DyCo3D/issues/8
    ...
    def forward(self, input, input_map, coords, batch_idxs, batch_offsets, epoch, ins_sample_num=70, training=True):
        ...
        if(epoch > self.prepare_epochs):
            ...
            object_idxs = torch.nonzero(semantic_preds > -1).view(-1)
            print("HHHHH", object_idxs.size(0))
            ...
            print(semantic_preds_cpu[:20], self.cluster_radius)
            ...
            print("No. of Proposals", proposals_offset_shift.size(0)-1)
...
def model_fn_decorator(test=False):
    ...
    def test_model_fn(batch, model, epoch):
        ...
        print("HHHH", ret['proposals_offset_shift'].size(0)-1)
        ##### preds
        with torch.no_grad():
            preds = {}
    ...
    def loss_fn(loss_inp, epoch):
        ...
        if (epoch > cfg.prepare_epochs):
            ...
            for n in range(inst_num):
                ...
                #     continue
                # culprit! this ignore semantic class <=1
                if ins_label_n < 0:
                    continue
