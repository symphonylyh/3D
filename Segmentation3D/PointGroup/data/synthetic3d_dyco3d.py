...
class Dataset:
    def __init__(self, start_iter=0, test=False):
    ...
    def trainLoader(self):
        ...
        if num_gpus > 1:
            ...
            images_per_gpu = self.batch_size // num_gpus

            # num_iters = int(len(self.train_files) / self.batch_size)
            # batch_sampler = make_batch_data_sampler(sampler, images_per_gpu, num_iters, 0)
            num_iters = self.max_iter
            batch_sampler = make_batch_data_sampler(sampler, images_per_gpu, num_iters, self.start_iter)
            data_args['batch_sampler'] = batch_sampler
            data_loader = DataLoader(**data_args)
        else:
            print('not distributed')
            # data_loader = DataLoader(train_set, batch_size=self.batch_size, collate_fn=self.trainMerge, num_workers=self.train_workers,
            #                                     shuffle=True, sampler=None, drop_last=True, pin_memory=True)
            data_args['sampler'] = InfSampler(train_set, shuffle=True)
            data_args['batch_size'] = cfg.batch_size
            data_loader = DataLoader(train_set, batch_size=self.batch_size, collate_fn=self.trainMerge, num_workers=self.train_workers,
                                                           shuffle=False, sampler=data_args['sampler'], drop_last=True, pin_memory=True)
    ...
    def trainMerge(self, id):
        ...
        for i, idx in enumerate(id):
            ...
            random_idx = np.random.permutation(len(xyz_origin))
            xyz_origin = xyz_origin[random_idx]
            rgb = rgb[random_idx]
            label = label[random_idx]
            instance_label = instance_label[random_idx]
